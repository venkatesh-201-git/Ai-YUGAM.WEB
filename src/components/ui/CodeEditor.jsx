...
const CodeEditor = ({ initialCode = '' }) => {
  ...
  return (
    <div className="flex flex-col w-full bg-[#0d0d0d] rounded-2xl md:rounded-3xl overflow-hidden shadow-2xl border border-white/5 transition-all max-w-5xl mx-auto">
      ...
      {/* Precision Header */}
      <div className="flex flex-wrap items-center justify-between px-2.5 sm:px-4 md:px-5 py-2 sm:py-2.5 bg-[#111] border-b border-white/5 gap-2">
         ...
         <div className="flex items-center gap-1.5 sm:gap-2">
            <div className="flex items-center gap-1 mr-1">
               <button onClick={() => navigator.clipboard.writeText(code)} aria-label="Copy Code" className="p-1.5 sm:p-2 text-gray-400 hover:text-white transition-all"><Copy className="w-3.5 h-3.5" /></button>
               <button onClick={() => setCode(initialCode)} aria-label="Reset Code" className="p-1.5 sm:p-2 text-gray-400 hover:text-white transition-all"><RotateCcw className="w-3.5 h-3.5" /></button>
            </div>
            <button onClick={handleRun} disabled={isCompiling} className="flex items-center gap-1.5 sm:gap-2 px-3 sm:px-6 py-1.5 sm:py-2 bg-secondary text-white text-[9px] sm:text-[10px] font-black uppercase tracking-widest rounded-xl shadow-xl shadow-secondary/10 active:scale-95 disabled:opacity-50">
               {isCompiling ? <div className="w-3 h-3 border-2 border-white/20 border-t-white rounded-full animate-spin" /> : <Play className="w-3.5 h-3.5 fill-current" />}
               <span>{isCompiling ? 'Running' : 'Run'}</span>
            </button>
         </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 h-[280px] sm:h-[380px] md:h-[480px] relative bg-[#0a0a0a]">
        <div className={`p-3 sm:p-6 ${activeTab === 'preview' && 'hidden lg:flex'}`}>
           <textarea
             value={code}
             onChange={(e) => setCode(e.target.value)}
             className="w-full h-full bg-transparent text-blue-50/90 font-mono text-[11px] sm:text-[13px] md:text-[15px] leading-relaxed outline-none resize-none scrollbar-hide caret-secondary"
             spellCheck="false"
           />
        </div>
        ...
        <div className={`bg-white relative border-l border-white/5 ${activeTab === 'editor' && 'hidden lg:flex'}`}>
           {isScript ? (
              <div className="w-full h-full bg-[#0a0a0a] p-3 sm:p-6 md:p-8 text-green-400 font-mono text-[10px] sm:text-xs md:text-sm overflow-y-auto flex flex-col">
                 <div className="flex-1 space-y-1.5">
                    {terminalHistory.map((item, i) => (
                       <div key={i} className={`flex gap-2 sm:gap-3 ${item.type === 'sys' ? 'text-blue-500 opacity-40' : item.type === 'in' ? 'text-yellow-400 font-bold' : 'text-green-500 font-black'}`}>
                          {item.type === 'out' && <span className="opacity-20 text-[9px] font-black">[{i}]</span>}
                 ...
                 {isWaitingForInput && (
                    <form onSubmit={submitInput} className="mt-auto border-t border-white/10 pt-3 flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-4">
                       <span className="text-secondary font-black text-[9px] sm:text-xs animate-pulse">?? INPUT REQUIRED {">>"}</span>
                       <input 
                         autoFocus
                         type="text" 
                         value={currentInput}
                         onChange={(e) => setCurrentInput(e.target.value)}
                         className="flex-1 bg-[#111] px-3 py-1.5 rounded-lg border border-white/10 outline-none text-white font-mono text-xs placeholder:text-gray-700 focus:border-secondary transition-all"
                         placeholder="Enter value and press Send..."
                       />
                       <button type="submit" className="p-2 sm:p-3 bg-secondary rounded-lg text-white hover:bg-blue-600 transition-all flex items-center justify-center"><Send className="w-4 h-4" /></button>
                    </form>
                 )}
              </div>
           ) : (
           ...
           <div className="absolute top-2.5 right-3 sm:top-4 sm:right-6 pointer-events-none px-2.5 py-0.5 sm:px-3 sm:py-1 bg-black/90 rounded-full border border-white/10 shadow-2xl">
              <span className="text-[7.5px] sm:text-[8px] font-black uppercase text-secondary tracking-widest">{langLabel}</span>
           </div>
        </div>
      </div>
      
      {/* HUD Footer */}
      <div className="px-3 sm:px-6 py-2 bg-[#080808] border-t border-white/5 flex justify-between items-center h-auto min-h-[2.5rem] flex-wrap gap-2">
         <div className="flex items-center gap-3 sm:gap-6 text-[7.5px] sm:text-[8px] font-black text-gray-600 uppercase tracking-widest">
            <div className="flex items-center gap-1.5"><Globe className="w-3 h-3 text-secondary/40" /><span>Sandbox OS v2</span></div>
            <div className="flex items-center gap-1.5"><Activity className="w-3 h-3 text-secondary/40" /><span>{code.length} Symbols</span></div>
         </div>
         <div className="flex items-center gap-2 flex-shrink-0">
            <span className="text-[7.5px] sm:text-[8px] font-black text-secondary tracking-widest uppercase">Kernel Operational</span>
